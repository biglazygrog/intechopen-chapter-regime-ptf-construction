...
**Decision.** The core universe's cash proxy was carried under the internal name
`high_yield`, a legacy label that names the wrong asset class: the series is
`LD12TRUU Index`, a Bloomberg US Short Treasury 1-12 Month index, and it has been
displayed as "Cash" in every table since the display maps were introduced. The
identifier is now `cash`, so the internal name and the display name agree. This
is a pure relabelling — no estimation logic was touched.

**Audit before touching anything.** 18 occurrences of `high_yield` across 8 files,
plus a binary sweep of every `.xlsx` and `.pdf` (each unzipped and searched:
zero hits, so no shared-strings table or PDF stream carries the identifier).

| Location | Occurrences | Disposition |
|---|---|---|
| `core/config.py:98,120,138` | 3 | Renamed — `CORE_ASSETS`, `ASSET_DISPLAY`, `CORRELATION_DISPLAY` |
| `core/utils.py:15,26` | 2 | Renamed — `CASH_LIKE_ASSETS` tuple and its explanatory comment |
| `data/data_master_v1.csv:6` | 1 | Renamed — `name` column only; `bbg_ticker` and `asset_block` untouched |
| `README.md:221` | 1 | Renamed — synchronous-trading filter exemption note |
| `research/output_charts/*.csv` | 6 | Regenerated by the re-run, not hand-edited |
| `REVISION_LOG.md` | 10 | **Left as-is** — historical entries are a record of what was decided when the identifier was `high_yield`; rewriting them would falsify the log |
| `data/data_master2.csv:9` | 1 | **Out of scope** — `LG30TRUU Index`, genuine high yield credit in the expanded universe |

Note for future readers: `CASH_LIKE_ASSETS` lives in `core/utils.py`, not
`core/config.py` where the other three identifiers sit.

**Collision risk found and resolved.** The expanded universe already contains an
asset named `cash` — `US0003M Index` at `data_master2.csv:14`, a genuinely
different series. `research/analysis/stability_analyzer.py` loads both masters and
concatenates them (`stability_analyzer.py:297-298`), dropping duplicate column
names, so the rename silently flips which series survives the collision:

- *Before:* core `high_yield` (LD12TRUU) beat expanded `high_yield` (LG30TRUU),
  which was dropped. Expanded `cash` (US0003M) survived as its own column.
- *After:* core `cash` (LD12TRUU) beats expanded `cash` (US0003M), which is now
  dropped. Expanded `high_yield` (LG30TRUU) becomes reachable.

**Table 5 is unaffected**, and this was checked rather than assumed:
`research/analysis/stability.py:46` drives its tiers from `EXTENDED_ADD_ORDER`,
which contains neither `cash` nor `high_yield`, so the nine tiers and their
numbers are untouched — confirmed by the byte-identical
`table5_universe_stability.tsv` below. The one affected site is the separate
`add_order` hardcoded in `stability_analyzer.py`'s `__main__` block, which ends
with `"cash"` meaning US0003M; after the rename that entry is skipped as
already-in-universe. That block is not in the README execution order and produces
no paper artefact, so no committed output changes. Per the author's instruction
the estimation code was left alone and a comment was added at the site recording
what the name means and that it needs updating if the core identifier changes
again.

**Re-run outcome: every headline number identical.** Full pipeline executed end
to end in README order (all 11 steps, every one `rc=0`), then compared against the
committed outputs on `main`.

| Strategy | Sharpe (committed) | Sharpe (re-run) | Max DD (committed) | Max DD (re-run) |
|---|---|---|---|---|
| Oracle (infeasible) | 1.154 | **1.154** | −14.8% | **−14.8%** |
| Unconditional | 0.799 | **0.799** | −14.7% | **−14.7%** |
| AR Baseline | 0.753 | **0.753** | −18.5% | **−18.5%** |
| AR Change | 0.753 | **0.753** | −18.2% | **−18.2%** |
| Random Walk | 0.648 | **0.648** | −21.8% | **−21.8%** |

**Maximum numerical disagreement across all 28 regenerated CSVs: exactly 0.**
Not "within tolerance" — 27 of the 28 files are *byte-for-byte identical* to the
committed version once `high_yield` → `cash` is applied to the committed copy.
(For comparison, the reviewer's independent reproduction on a different stack
reported 1.5e-10; a relabelling should and did produce nothing at all.)

The 28th file, `paper_profiles_raw.csv`, differs only by **row order**: its rows
are ordered alphabetically by asset — `paper_profiles` is built by
`prof_low.join(prof_high, how="outer")` at `models/regime_analysis.py:857`, and a
pandas outer join sorts on the joined index — so the row moved from position 3 to
position 1 when `high_yield` became `cash`. Sorting both files on the asset key
makes them byte-identical: every value is unchanged. Its only consumer,
`research/analysis/moments.py`, reorders by `DISPLAY_ORDER` regardless, which is
why `table3_regime_moments.tsv` is byte-identical.

**Caveat — the re-run environment is not the verified stack.** README pins
Python 3.13.11 / numpy 2.5.2 / pandas 3.0.5; this run was Python 3.12.1 /
numpy 2.5.1 / pandas 3.0.5. The zero disagreement is therefore a stronger result
than it looks (it held across a minor version drift), but the verified-environment
claim in the README still rests on the reviewer's run, not this one.

**Open flag for the author — Table 1.** The README artefact map records Table 1
(Baseline Asset Universe) as hand-written from `data/data_master_v1.csv`. If the
manuscript's Table 1 prints the internal `name` column rather than the display
label, the chapter text now disagrees with the data file and needs a one-word
edit. The chapter `.docx` is not in this environment, so this could not be
checked here.
